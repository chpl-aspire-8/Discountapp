package com.oterrionboarding.customer.utils.networkvalidator
...
class NetworkValidator(private val context: Context) {
    ...
    fun isOnline(): Boolean {
        ...
        network?.let {
            ...
            networkCapabilities?.let { it1 ->
                isOnline =
                    (it1.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR)) || (it1.hasTransport(
    ...
    fun isWifiAvailable(): Boolean {
        connectivityManager.apply {
            getNetworkCapabilities(activeNetwork)?.let {
                return it.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) && it.hasCapability(
    ...
    fun isCellularAvailable(): Boolean {
        return telephonyManager.dataState != TelephonyManager.DATA_DISCONNECTED
    }
